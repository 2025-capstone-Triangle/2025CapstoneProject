import { Bell, ChevronLeft, ChevronRight, Globe, Lock, LogOut, Palette, Trash2, User } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left">
        <p className={`font-['Noto_Sans_KR'] text-[15px] font-medium ${danger ? "text-red-600" : "text-black"}`}>
          {label}
        </p>
        {description && <p className="font-['Noto_Sans_KR'] text-[12px] text-gray-500 mt-0.5">{description}</p>}
      </div>
...
function SettingSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      ...
      <div className="bg-white border-y border-gray-100">{children}</div>
    </div>
  );
}
  ...
  return (
    <div className="bg-gray-50 min-h-screen max-w-[390px] mx-auto">
      ...
      <div className="py-4">
        ...
        <SettingSection title="앱 설정">
          <SettingItem
            ...
            label="언어"
            description="한국어"
            onClick={() => alert("언어 설정 기능은 준비 중입니다.")}
          />
          <SettingItem
            icon={Palette}
            label="테마"
            description="라이트 모드"
            onClick={() => alert("테마 설정 기능은 준비 중입니다.")}
          />
        </SettingSection>

        <SettingSection title="기타">
          <SettingItem icon={LogOut} label="로그아웃" onClick={handleLogout} />
          <SettingItem
            icon={Trash2}
            label="계정 삭제"
      ...
      <div className="px-6 py-8 text-center">
        <p className="font-['Noto_Sans_KR'] text-[12px] text-gray-400 mb-2">Person:a v1.0.0</p>
        <p className="font-['Noto_Sans_KR'] text-[11px] text-gray-400">© 2026 Person:a. All rights reserved.</p>
      </div>
    </div>
  );
}
