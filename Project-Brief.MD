...
| 사용할 소프트웨어 패키지의 명칭과 핵심기능/용도 | - [backend]	SpringBoot	(웹 애플리케이션 개발 프레임워크), AWS EC2(클라우드 호스팅), AWS S3	(대용량 파일 저장소), Docker	(애플리케이션 패키징 및 배포) - [frontend] React	(UI 구축
), Tailwind CSS	(웹 디자인 및 스타일링) -[AI] Python, Librosa, pyAudioAnalysis	(음성 분석), PyTorch	(딥러닝 모델 개발), Langchain + Gemini (자연어 분석), Adobe Firefly (AI 이미지 생성) |
