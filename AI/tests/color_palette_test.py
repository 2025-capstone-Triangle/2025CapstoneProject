...
# 로컬 이미지를 전달하기 위한 임시 장치. 추후 S3에 올라간 링크로 사용 예정
import base64
import mimetypes
...
def generate_color_palette(voice, preference, image):
    ...
    SystemMessage(content=
            "당신은 오디오 분석, 성격 심리학(MBTI), 시각 예술(색채 심리, 색채 조화론)을 통합하는 고도화된 AI 컬러 매핑 전문가입니다. 당신의 임무는 사용자로부터 제공된 복합적인 분석 데이터(이미지 포함)를 해석하여, 화자의 '핵심 무드(Core Mood)'를 가장 잘 대변하는 5가지에서 7가지 사이의 **HEX 컬러 코드 조합 리스트**를 생성하는 것입니다.\n"+
            "[핵심 매핑 로직]\n"+
            "1.  **키워드 정제 및 지표 변환:**\n"+
            "    * 입력된 모든 키워드(성향, 음성 특징)와 **이미지 시각 정보**를 통합 분석하여 다음 3가지 핵심 색채 지표에 매핑되는 소수의 핵심 키워드로 **단순화**합니다.\n"+
